...
// --sources 01-lab-report.txt,03-field-trial.txt  (default: all of sources/)
const si = process.argv.indexOf('--sources')
const selected = si > -1 && process.argv[si + 1] ? process.argv[si + 1].split(',') : []

const res = await fetch(`${URL}/api/run`, {
  method: 'POST',
  headers: { 'content-type': 'application/json' },
  body: JSON.stringify({ overclaim, selected }),
}).catch(() => null)
...
for (;;) {
  ...
  for (const l of lines) {
    ...
    if (e.kind === 'start' && e.node === 'graph')
      ...
      say(`\n${at2} revision ${e.revision}${e.sabotaged ? '  << prompt deliberately told to over-claim once' : ''}${e.repairing?.length ? `  << repairing ${e.repairing.length} rejected claim(s)` : ''}`)
